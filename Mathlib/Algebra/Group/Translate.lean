...
variable {ι α β M G H R : Type*} [AddCommGroup G]
...
@[inherit_doc] notation "τ " => translate
...
@[simp] lemma translate_zero_right (a : G) : τ a (0 : G → M) = 0 := rfl

lemma translate_add_right (a : G) (f g : G → M) : τ a (f + g) = τ a f + τ a g := rfl
...
lemma translate_sub_right (a : G) (f g : G → H) : τ a (f - g) = τ a f - τ a g := rfl
lemma translate_neg_right (a : G) (f : G → H) : τ a (-f) = -τ a f := rfl
...
lemma translate_prod_right (a : G) (f : ι → G → R) (s : Finset ι) :
    τ a (∏ i in s, f i) = ∏ i in s, τ a (f i) := by ext; simp
